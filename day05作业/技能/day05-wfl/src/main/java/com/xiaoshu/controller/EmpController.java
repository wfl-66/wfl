...
public class EmpController extends LogController{
	...
	public void userList(EmpVo ev,HttpServletRequest request,HttpServletResponse response,String offset,String limit) throws Exception{
		try {
			Integer pageSize = StringUtil.isEmpty(limit)?ConfigUtil.getPageSize():Integer.parseInt(limit);
			Integer pageNum =  (Integer.parseInt(offset)/pageSize)+1;
			PageInfo<EmpVo> userList= es.findUserPage(ev,pageNum,pageSize);
	...
	public void reserveUser(MultipartFile picc , HttpServletRequest request,Emp emp,HttpServletResponse response) throws IllegalStateException, IOException{
		Integer userId = emp.geteId();
		JSONObject result=new JSONObject();
		String filename = picc.getOriginalFilename();
		if(filename!=null){
			String newFilename = UUID.randomUUID().toString()+filename.substring(filename.lastIndexOf("."));
			picc.transferTo(new File("d:/pic/"+newFilename));
