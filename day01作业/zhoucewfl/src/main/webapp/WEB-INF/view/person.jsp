...
<body class="gray-bg">
        ...
        <div class="row">
			  ...
			  <div class="col-lg-2">
				<div class="input-group">
					...
					<select class="form-control" name="expressTypeId" id = "txexpressTypeId">
						<option value="0">---请选择---</option>
							<c:forEach items="${cList }" var="r">
							 	<option value="${r.id }">${r.expressEname }</option>
							</c:forEach>
                	</select>
				</div>
			 </div>
				...
				<div class="modal-body">
					<form id="form_user" method="post" action="reservePerson.htm">
						...
						<table style="border-collapse:separate; border-spacing:0px 10px;">
							...
							<tr>
								<td>人员特点：</td>
								<td><input type="text" id="expressTrait" name="expressTrait"
									class="form-control" aria-required="true" required/></td>
								<td>&nbsp;&nbsp;</td>
								<td>入职时间：</td>
	...
	var TableInit = function () {
	    ...
	    oTableInit.queryParams = function (params) {
	        var temp = {//这里的键的名字和控制器的变量名必须一致，这边改动，控制器也需要改成一样的
	            ...
	            expressTypeId: $("#txexpressTypeId").val(),
	...
	function initEditUser(getSelection){
		...
		$('#expressTrait').val(getSelection.expressTrait);
		$('#password').attr("readOnly",true).val(getSelection.entryTime);
